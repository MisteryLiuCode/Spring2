...
//    定义一个切入点，可以达到一个表达式复用
    @Pointcut(value = "execution(public Float com.liu.spring.aop.Cat.getSum(Float, Float))")
    public void myPoint(){
    }
...
//        获取参数
    ...
    //只有发生异常的时候才会执行
    @AfterThrowing(value = "execution(public Float com.liu.spring.aop.Cat.getSum(Float, Float))", throwing = "e")
...
//        获取参数
    ...
    //最终通知,不管是否发生异常都要执行.
    @After(value = "execution(public Float com.liu.spring.aop.Cat.getSum(Float, Float))")
...
//    //    对没有实现接口的类使用切面编程
//    @Before(value = "execution(public void Cat.getSum())")
